...
export const createBookingAccCtrl = async (req, res) => {
	let result = null;
	let isValid = true;
	let isEmpty = false;
	const newData = req.body;
	const { email, phone } = constVals.nameKeysValues;
	...
	for (const key in newData) {
		...
		if (key === email) {
			...
			isValid = Commons.numberRegex(newData[key]);
			if (isValid === false) break;
		}
	}
	...
	if (isEmpty) {
		...
		try {
			result = await userServs.createBookingAccServ(newData);
		} catch (error) {
			console.log('createBookingAccCtrl error', error);
		}
	}
	...
	if (id && typeof id === constVals.typeKeyValue.numType) {
		...
		if (isEmpty) {
			...
			try {
				...
				console.log('updateRefreshTokenCtrl error', error);
			}
		}
	}

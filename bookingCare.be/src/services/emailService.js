...
export const sendEmailWithAttachment = async (emailInfo) => {
	return new Promise(async (resolve, reject) => {
		try {
			const transporter = nodemailer.createTransport(smtpConfig);
			let result = false;
			if (transporter) {
				const { clientEmail, clientSubject, attachments } = emailInfo;
				const webSite = 'Bookingcare.vn';
				let htmlText = getEmailContentForPatientBill(emailInfo);
...
const getEmailContentForPatientBill = (emailInfo) => {
	let htmlText = null;
	const { clientEmail, enLang } = emailInfo;

	if (enLang) {
		htmlText = `<h3>Hi! ${clientEmail}.</h3>
		This is your bill at Booking Care hopital.
  `;
	} else {
		htmlText = `<h3>Xin chào! ${clientEmail}.</h3>
  Đây là hóa đơn thanh toán của bạn tại bệnh viện Booking Care`;
	}
...
const getEmailContentForPassword = (emailInfo) => {
	...
	if (enLang) {
		htmlText = `<h3>Hi! ${clientEmail}.</h3>
  Your a reset password on the Bookingcare.vn website. Note, available link is about ${
			timeLimited / 60
		}hs

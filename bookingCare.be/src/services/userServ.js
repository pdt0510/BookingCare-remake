	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			const user = await db.bookings.findOne({
				where: { email: newData.email, dateBooked: newData.dateBooked },
			});
			...
			if (user) {
				...
				const isCreated = await db.bookings.create(newData);
...
export const sendEmailToGetTokenServ = (emailData) => {
	return new Promise(async (resolve, reject) => {
		try {
			let result = apiStates.notFound;
			const success = 1;
			const timeLimited = '120m'; //minutes
...
export const updateUserByIdServ = (newData) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (user) {
				const isUpdate = !newData.password;
				const hashedPassword = isUpdate ? undefined : await convertToHashedPassword(newData.password);
				const success = 1;
				const dataForUpdate = {
					...newData,
					id: undefined,
...
export const getUserByIdServ = (id) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			const user = await db.users.findOne({
				where: { id },
				attributes: { exclude: ['password', 'createdAt', 'updatedAt', 'resetPasswordToken'] },
			});
